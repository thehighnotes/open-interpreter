...
| `prepare` | — | Session setup: wake hosts, warm LLM, start services |
...
hub --vllm [status|start|stop|restart]
...
1. **HOSTS** — online/offline, uptime, RAM usage, loaded LLM model
...
# Let the LLM write the commit message:
repo commit myapp
```

When you omit the message, `repo` sends the `git diff --staged` output to the LLM and gets back a conventional commit message. The diff is sent via SCP + curl (not shell escaping) to handle large diffs cleanly.
...
work <project> --force          Force cache refresh (bypass smart skip)
```

Runs `prepare`, then `overview`, then shows a 5-second countdown (Ctrl+C to cancel) before launching `begin`. Passes flags through to downstream tools (including `--force` to `prepare`).
...
```
prepare <project>               Wake host, warm LLM backend, start services, refresh cache
prepare <project> --no-wake     Skip Wake-on-LAN, just report if host is suspended
prepare <project> --force       Force cache refresh (bypass smart skip)
```

Four-step flow:
1. **Check host** — SSH probe; sends Wake-on-LAN if the host has the `wakeable` role (waits up to 120s)
2. **Warm LLM backend** — background curl to load the model into GPU memory
...
**Smart skip:** Before refreshing the cache, `prepare` probes the project host for three staleness indicators — git HEAD, CLAUDE.md mtime, and transcript mtime — in a single SSH call (~0.5s). If all three match the stored state from the last successful refresh and the cache is less than 6 hours old, the expensive LLM analysis, decision extraction, and repo extraction are skipped entirely. Use `--force` to bypass.
...
Continuous research monitoring that fetches papers and releases, scores them against your projects via LLM. Automatically discovers relevant repos from your development sessions.
...
Items are scored 0-10 for relevance against each project simultaneously via a single LLM call per item:
...
```
  [10] ↑ ratatui/ratatui ratatui-v0.30.0
       ...
       Direct dependency — ratatui is the TUI framework for this project
       → Update ratatui dependency to v0.30.0 for new widget APIs
```

### Status

`research --status` shows fetch timestamps, cache stats, age distribution, per-project digest summaries, per-project tracked repos, and LLM service health.
...
2. LLM service (vLLM on port 8000, Ollama on port 11434)
