...
Color coding: green (<60%), yellow (60–80%), red (>80%). The `context_window` value is set in your profile or `config.json`. For Ollama backends, it's also passed as `num_ctx` so the server allocates the right KV cache. For vLLM, the context window is fixed at server start via `--max-model-len`.
...
## Inline edit block interceptor

The OI profile wraps `interpreter.computer.run` with a `_clean_run` function that intercepts multi-line `~/edit` blocks before they reach the shell. Without this, content containing parentheses, quotes, or other shell metacharacters would cause bash syntax errors.

The interceptor detects the `~/edit <file> <range>` pattern on the first line, parses the target and range spec, and applies the edit entirely in Python using the `~/edit` module's `read_file` / `write_file` functions. The LLM sees structured diff feedback (removed/inserted lines, line counts, duplicate warnings) instead of shell errors.

Additionally, the interceptor:
- **Strips `\r` line endings** — Qwen models occasionally emit `\r\n`; these are normalized to `\n`
- **Injects `--exclude-dir`** into recursive `grep` commands to skip `.git`, `node_modules`, `__pycache__`, `.venv`, etc.
- **Blocks interactive SSH** — bare `ssh agx` (no command) would hang the PTY; replaced with an error message
- **Tracks hub state changes** — prints a subtle indicator after git commits, pushes, and backups
