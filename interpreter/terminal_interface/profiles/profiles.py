...
def profile(interpreter, filename_or_url):
    # See if they're doing shorthand for a profile name (without extension)
    filename_without_extension = os.path.splitext(filename_or_url)[0]
    if filename_without_extension == filename_or_url:
        # No extension provided — check default profiles first
        for profile in default_profiles_names:
            if filename_without_extension == os.path.splitext(profile)[0]:
                filename_or_url = profile
                break
        else:
            # Check user profile directory for matching files
            for ext in ('.py', '.yaml', '.json'):
                candidate = os.path.join(profile_dir, filename_or_url + ext)
                if os.path.exists(candidate):
                    filename_or_url = filename_or_url + ext
                    break
