...
def fixed_litellm_completions(**params):
    ...
    # Disable extended thinking for local models (qwen3.5 burns all tokens
    # on internal reasoning and returns empty content without this)
    _model = params.get("model", "")
    if _model.startswith("ollama/") and "qwen" in _model.lower():
        params["reasoning_effort"] = "none"
    elif _model.startswith("openai/") and "qwen" in _model.lower():
        # vLLM rejects reasoning_effort="none" (only accepts low/medium/high)
        # Use chat_template_kwargs to suppress thinking at the template level
