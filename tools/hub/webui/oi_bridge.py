...
class OIBridge:
    ...
    def initialize(self):
        ...
        if _LLM_BACKEND == 'vllm':
            interpreter.llm.model = f"openai/{self._model_name}"
        else:
        ...
        try:
            if _LLM_BACKEND == 'vllm':
                req = urllib.request.urlopen(f"{_LLM_BASE}/models", timeout=3)
                data = json.loads(req.read())
                models = [m["id"] for m in data.get("data", [])]
            else:
                req = urllib.request.urlopen(f"{_LLM_BASE}/api/tags", timeout=3)
                data = json.loads(req.read())
    ...
    def update_model(self, model_name):
        ...
        if self._interpreter:
            ...
            if _LLM_BACKEND == 'vllm':
                self._interpreter.llm.model = f"openai/{model_name}"
            else:
