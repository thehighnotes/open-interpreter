...
def _resolve_llm_config(config):
    """Resolve LLM config from 'llm' section, falling back to 'ollama' for backward compat."""
    if 'llm' in config:
        llm = config['llm']
        return {
            'backend': llm.get('backend', 'vllm'),
            'host': llm.get('host', 'local'),
            'port': llm.get('port', 8000),
            'model': llm.get('model', 'llama3.2:3b'),
            'context_window': llm.get('context_window', 44000),
        }
    # Backward compat: derive from ollama section
    ollama = config.get('ollama', {})
    return {
        'backend': 'ollama',
        'host': ollama.get('host', 'local'),
        'port': ollama.get('port', 11434),
        'model': ollama.get('default_model', 'llama3.2:3b'),
        'context_window': ollama.get('num_ctx', 44000),
    }


def reload_config():
    """Re-read config from disk and update all module-level globals in-place."""
    global LOCAL_HOST, ROLE, HUB_HOST
    global LLM_BACKEND, LLM_HOST, LLM_PORT, LLM_BASE_URL, DEFAULT_MODEL, LLM_CONTEXT_WINDOW
    ...
    # LLM config (vllm or ollama)
    _llm = _resolve_llm_config(HUB_CONFIG)
    LLM_BACKEND = _llm['backend']
    LLM_HOST = _llm['host']
    LLM_PORT = _llm['port']
    DEFAULT_MODEL = _llm['model']
    LLM_CONTEXT_WINDOW = _llm['context_window']

    # Backward compat aliases
    OLLAMA_HOST = HUB_CONFIG.get('ollama', {}).get('host', LLM_HOST)
    OLLAMA_PORT = HUB_CONFIG.get('ollama', {}).get('port', 11434)
    ...
    # Resolve LLM base URL from host IP
    _llm_ip = HOSTS.get(LLM_HOST, {}).get('ip', '127.0.0.1')
    LLM_BASE_URL = f'http://{_llm_ip}:{LLM_PORT}'
...
# LLM config (vllm or ollama — resolved from 'llm' section with 'ollama' fallback)
_llm_init = _resolve_llm_config(HUB_CONFIG)
LLM_BACKEND = _llm_init['backend']
LLM_HOST = _llm_init['host']
LLM_PORT = _llm_init['port']
DEFAULT_MODEL = _llm_init['model']
LLM_CONTEXT_WINDOW = _llm_init['context_window']

# Backward compat aliases (used by some tools that reference OLLAMA_HOST directly)
OLLAMA_HOST = HUB_CONFIG.get('ollama', {}).get('host', LLM_HOST)
OLLAMA_PORT = HUB_CONFIG.get('ollama', {}).get('port', 11434)
...
# LLM base URL — resolved from host IP (available after HOSTS is built)
_llm_ip = HOSTS.get(LLM_HOST, {}).get('ip', '127.0.0.1')
LLM_BASE_URL = f'http://{_llm_ip}:{LLM_PORT}'
...
def resolve_project(query):
    ...
    and path basenames (e.g. matches directory names to project keys).
...
# ─────────────────────────────────────────────────────────────────────────────
# LLM query (unified — used by overview + research)
# Supports vLLM (direct HTTP, OpenAI format) and Ollama (SCP+SSH, /api/chat)
# ─────────────────────────────────────────────────────────────────────────────

def _llm_query_vllm(model, messages, timeout, temperature, max_tokens, json_mode=False):
    """Query vLLM via direct HTTP (OpenAI-compatible). Returns raw response text or None."""
...
def _llm_query_ollama(model, messages, timeout, temperature, max_tokens, json_mode=False):
    """Query Ollama via SCP+SSH (or locally). Returns raw response text or None."""
    import os as _os
...
def llm_query(model, system_msg, user_msg, timeout=120, temperature=0.3, num_predict=2048):
    """Send a query to the configured LLM backend. Returns parsed JSON dict or None.

    Routes to vLLM (direct HTTP) or Ollama (SCP+SSH) based on LLM_BACKEND config.
    """
    messages = [
    ...
    if LLM_BACKEND == 'vllm':
        text = _llm_query_vllm(model, messages, timeout, temperature, num_predict)
    else:
        text = _llm_query_ollama(model, messages, timeout, temperature, num_predict, json_mode=True)
...
def llm_query_text(model, system_msg, user_msg, timeout=60, temperature=0.3, num_predict=256):
    """Send a query to the configured LLM backend. Returns raw text string or None."""
    messages = [
        {'role': 'system', 'content': system_msg},
        {'role': 'user', 'content': user_msg},
    ]
    ...
    if LLM_BACKEND == 'vllm':
        ...
        text = _llm_query_ollama(model, messages, timeout, temperature, num_predict, json_mode=False)
...
def ollama_query(model, system_msg, user_msg, timeout=120, temperature=0.3, num_predict=2048):
    """Backward compat wrapper for llm_query."""
    return llm_query(model, system_msg, user_msg, timeout, temperature, num_predict)


def ollama_query_text(model, system_msg, user_msg, timeout=60, temperature=0.3, num_predict=256):
    """Backward compat wrapper for llm_query_text."""
    return llm_query_text(model, system_msg, user_msg, timeout, temperature, num_predict)
...
def extract_repos_from_transcript(project_key):
    ...
    if LLM_HOST == LOCAL_HOST or check_host_reachable(LLM_HOST):
        system_msg = """/no_think
...
def rsync_to_ca_host(source_host, source_path, mirror_path, dry_run=False):
    ...
    Args:
        ...
        mirror_path: Target path on CA host (e.g. ~/projects/myapp)
