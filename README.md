...
This fork fixes those things. The core improvements — callable auto_run, Mini-RAG, vision, output handling — work on any single machine with vLLM or Ollama. No multi-machine setup required.
...
- **Project context injection** — when launched via `work --oi`, OI receives the project's directory tree, CLAUDE.md conventions, and session preamble. Full CLAUDE.md is pre-seeded as silent context; a truncated version stays in the system message
- **Cross-shell PTY** — shell subprocess works on both bash and zsh hosts, with bracketed paste mode disabled at the terminal level
...
```bash
# Hub — full install (all state, cron, tools)
curl -sL https://raw.githubusercontent.com/thehighnotes/open-interpreter/main/tools/hub/bootstrap.sh | bash

# Node — OI + delegation to an existing hub
curl -sL https://raw.githubusercontent.com/thehighnotes/open-interpreter/main/tools/hub/bootstrap.sh | bash -s node
```
...
cd open-interpreter
pip install -e .
python3 tools/hub/install.py             # hub mode
python3 tools/hub/install.py --node      # node mode
```

For OI improvements only (no hub tools), you're done after `pip install` — create a [profile](docs/hub/getting-started.md#quick-start) pointing at your LLM server (vLLM or Ollama) and run `interpreter --profile my-profile.py`.

Keep up to date with `interpreter --update` (or enable `oi_auto_update` in config for automatic pulls on startup).

See the [Getting Started guide](docs/hub/getting-started.md) for configuration details, LLM profiles, and the full `config.json` schema.
...
Development happens across three machines — a workstation for coding, an AGX Orin running vLLM for inference, and an Orin Nano as the always-on hub. The tools started as shell scripts to avoid repetitive SSH sessions and kept evolving as new problems came up. Born on ARM + NVIDIA but designed to be architecture-generic — pure Python, SSH, and config files. If you can run Python 3.10+ and reach your machines over SSH, it works.
...
| Page | Content |
|------|---------|
| [Getting Started](docs/hub/getting-started.md) | Install, config schema, LLM profiles, host roles |
...
| [Troubleshooting](docs/hub/troubleshooting.md) | LLM backend, Code Assistant, WebUI — common problems and fixes |
